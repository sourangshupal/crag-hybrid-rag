...
This document describes the CI/CD pipeline for the **CRAG Hybrid RAG** application.
...
| ECR Repository | `crag-hybrid-rag` |
| ECR Image URL | `<ACCOUNT_ID>.dkr.ecr.us-east-1.amazonaws.com/crag-hybrid-rag:latest` |
| EB Application | `crag-hybrid-rag` |
| EB Environment | `crag-hybrid-prod` |
| S3 Bucket (EB) | `elasticbeanstalk-us-east-1-<ACCOUNT_ID>` |
| Instance Type | `t3.large` (single instance) |
| Platform | Docker on 64-bit Amazon Linux 2 v4.5.3 |

> Replace `<ACCOUNT_ID>` with your real AWS account ID after running `aws sts get-caller-identity`.
...
├── .ebextensions/
│   └── storage.config               # EBS volume size (30 GB gp3)
├── .platform/
...
docker build --platform linux/amd64 \
  -t <ACCOUNT_ID>.dkr.ecr.us-east-1.amazonaws.com/crag-hybrid-rag:<SHA> \
  -t <ACCOUNT_ID>.dkr.ecr.us-east-1.amazonaws.com/crag-hybrid-rag:latest .
```
...
```
The bundle contains only the EB configuration files. The actual application runs from the ECR image
declared in `Dockerrun.aws.json`. The `Dockerfile` is excluded (handled by `.ebignore`).

#### 6. Upload Bundle to S3
Uploads `deploy.zip` to the EB-managed S3 bucket under `crag-hybrid-rag/<version-label>.zip`.
...
aws elasticbeanstalk update-environment \
  --environment-name crag-hybrid-prod \
...
Polls every 30 seconds for up to 20 minutes. The AWS built-in waiter is not used because it has a
hardcoded ~6.5 minute limit, which is too short for Docker deployments.
...
| `TAVILY_API_KEY` | Tavily API key for web search (CRAG mode) |
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": "arn:aws:s3:::elasticbeanstalk-us-east-1-<ACCOUNT_ID>/*"
    },
    {
      "Effect": "Allow",
      "Action": [
...
| Variable | Purpose |
|---|---|
| `OPENAI_API_KEY` | LLM inference and embeddings |
| `TAVILY_API_KEY` | Web search fallback (CRAG mode) |
...
EB uses this file to pull the Docker image from ECR. The `latest` tag is always used so EB picks up
the newest image on every deploy.

```json
{
  "AWSEBDockerrunVersion": "1",
  "Image": {
    "Name": "<ACCOUNT_ID>.dkr.ecr.us-east-1.amazonaws.com/crag-hybrid-rag:latest",
    "Update": "true"
  },
  "Ports": [
    { "ContainerPort": 8000 }
...
```bash
curl http://crag-hybrid-prod.<region>.elasticbeanstalk.com/health
...
| EB parser breaks on Dockerfile | EB misreads Python `from` imports as Docker `FROM` | Excluded `Dockerfile` via `.ebignore` |
